...
char *CDCTrfSetupHandler(setup_packet*);
void CDCCtrlTrfSetupComplete(setup_packet*);
...
namespace {
    ...
    bool USBCtrlTrfSetupHandler(char* &phy_buffer) {
        ...
        if (SetupPkt.type == TYPE_CLASS)
            return phy_buffer = virt2phy(CDCTrfSetupHandler(&SetupPkt));
        return false;
    }
    ...
    void USBCtrlTrfSetupComplete(void) {
        if (SetupPkt.type == TYPE_STANDARD) {
            switch (SetupPkt.request) {
                ...
                case SET_CONFIGURATION:
                    reset_non_zero_endpoint();
                    CDCInitEndpoint(USBActiveConfiguration = SetupPkt.value);
                    break;
                default:;
            }
        }
        if (SetupPkt.type == TYPE_CLASS) CDCCtrlTrfSetupComplete(&SetupPkt);
        prepare_for_setup();
    }
...
_USB1Interrupt(void){
    ...
    while (U1IRbits.TRNIF) {
        if (U1STAT & STAT_ENDPT) CDC_TRN_Handler(bdt[U1STAT >> 2].bytecount);
